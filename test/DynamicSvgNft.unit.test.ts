...
!developmentChains.includes(chainId)
    ...
    : describe("Dynamic SVG NFT Unit Test", () => {
          ...
          beforeEach(async () => {
              ...
              tokenContract = await deployments.get("DynamicSvgNft");
          ...
          describe("mint nft", () => {
              ...
              it("shifts the token uri to lower when the price doesn't surpass the highvalue", async function () {
                  const highValue = parseEther("100000");
          ...
          describe("tokenURI", () => {
              it("sould revert when tokenId not provided", async () => {
                  await expect(
                      dynamicSvgNft.tokenURI(99)
